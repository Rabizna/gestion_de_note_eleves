  ...
  const updateNumbers = async () => {
    try {
      const result = await Swal.fire({
        title: "Mise à jour des numéros",
        html: `
          <div style="text-align:left;">
            <p style="margin-bottom:15px;">Voulez-vous mettre à jour les numéros pour :</p>
            <ul style="margin:10px 0; padding-left:20px;">
              <li style="margin-bottom:10px;"><strong>Toutes les classes</strong> : Réattribue les numéros à tous les élèves inscrits</li>
              <li><strong>Une classe spécifique</strong> : Uniquement pour le niveau/section filtré actuellement</li>
            </ul>
          </div>
        `,
        icon: "question",
        ...
        showDenyButton: true,
        confirmButtonText: "Toutes les classes",
        denyButtonText: "Classe filtrée",
      ...
      if (result.isDenied) {
        if (!niveauId || !sectionId) {
          Swal.fire({
            icon: "warning",
            title: "Attention",
            text: "Veuillez sélectionner un niveau ET une section pour cette option.",
          });
          return;
        }
        body.niveauId = Number(niveauId);
        body.sectionId = Number(sectionId);
      }

      const res = await api("/api/inscription/update-numbers", {
        method: "POST",
      ...
      Swal.fire({
        icon: "success",
        title: "Numérotation mise à jour !",
        text: `${res.updated} élève(s) renuméroté(s)`,
        confirmButtonColor: "#0ea5e9",
      });
      ...
      Swal.fire({
        icon: "error",
        title: "Erreur",
        text: e.message || "Impossible de mettre à jour les numéros.",
      });
    }
  };
    ...
    <>
      ...
      <div className="wrap">
        ...
        <div className="filters">
          <button className="btn btn-muted" onClick={() => navigate("/dashboard/inscription")}>
            ← Retour
          </button>
          ...
          <button className="btn btn-orange" onClick={updateNumbers} title="Mettre à jour la numérotation">
            🔢 Numéroter
          </button>

          <span style={{ marginLeft: "auto", color: "#f1f5f9" }}>
            Total: <b>{total}</b>
          </span>
        </div>
        ...
        <div className="panel">
          {loading ? (
            ...
            <table className="table">
              <thead>
                <tr>
                  <th style={{ width: "80px" }}>N° Classe</th>
                  <th>Élève</th>
                  <th>Sexe</th>
                  <th>Niveau</th>
                  <th>Section</th>
              ...
              <tbody>
                {items.map((e) => (
                  <tr key={e.id}>
                    <td>
                      <span className="num-badge">{e.numero ?? "-"}</span>
                    </td>
