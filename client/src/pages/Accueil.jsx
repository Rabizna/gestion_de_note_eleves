...
// Transforme n'importe quelle forme de payload en tableau
function toArray(payload, keys = ["items", "data", "rows", "niveaux", "sections"]) {
...
/* ===== Helpers filtrage niveau/section (UI only) ===== */
function normalize(str) {
  return String(str || "")
    .normalize("NFD").replace(/[\u0300-\u036f]/g, "") // retire accents
    .toLowerCase().trim();
}
function isSeconde(nomNiveau) {
  const n = normalize(nomNiveau);
  return n.includes("seconde");
}
function isPremiereOrTerminale(nomNiveau) {
  const n = normalize(nomNiveau);
  return n.includes("premiere") || n.includes("terminale");
}
function sectionCode(sectionNom) {
  const n = normalize(sectionNom);
  // on mappe vers nos codes simples
  if (n === "a") return "a";
  if (n === "b") return "b";
  if (n === "c") return "c";
  if (n === "l") return "l";
  if (n === "s") return "s";
  if (n === "ose") return "ose";
  // accepte "serie l", "serie s", etc.
  if (/\bserie\s*l\b/.test(n)) return "l";
  if (/\bserie\s*s\b/.test(n)) return "s";
  return n; // fallback
}
function sectionAllowedForNiveau(niveauNom, sectionNom) {
  const code = sectionCode(sectionNom);
  if (isSeconde(niveauNom)) {
    return code === "a" || code === "b" || code === "c";
  }
  if (isPremiereOrTerminale(niveauNom)) {
    return code === "l" || code === "s" || code === "ose";
  }
  // autre niveau → on ne contraint pas
  return true;
}
...
export default function Accueil() {
  ...
  // === Titularisation (uniquement pour PROFESSEUR) ===
  const [niveaux, setNiveaux] = useState([]);     // toujours un []
  theEmail: {}
  const [sections, setSections] = useState([]);   // toujours un []
  const [titulaireNiveauId, setTitulaireNiveauId] = useState(null);
  const [titulaireSectionId, setTitulaireSectionId] = useState(null);
  ...
  const isProviseur = roleUpper === "PROVISEUR";
  const isProf = roleUpper === "PROFESSEUR";

  // Chargement /me + listes
  useEffect(() => {
    let mounted = true;
    ...
    api("/api/auth/me")
      .then(({ user }) => {
        ...
        // valeurs de titularisation (si renvoyées par l’API)
        setTitulaireNiveauId(user.titulaireNiveauId ?? null);
        setTitulaireSectionId(user.titulaireSectionId ?? null);
    ...
    Promise.allSettled([api("/api/niveaux"), api("/api/sections")]).then(
      ([niv, sec]) => {
        if (!mounted) return;
        if (niv.status === "fulfilled") setNiveaux(toArray(niv.value));
        if (sec.status === "fulfilled") setSections(toArray(sec.value));
      }
    );
  ...
  // ====== Filtrage des sections selon le niveau (UI only) ======
  const currentNiveauNom = (() => {
    const n = (Array.isArray(niveaux) ? niveaux : []).find(
      (x) => (x.id ?? x.id_niveau) === titulaireNiveauId
    );
    return n?.nom ?? n?.nom_niveau ?? "";
  })();

  const filteredSections = (Array.isArray(sections) ? sections : []).filter((s) =>
    sectionAllowedForNiveau(currentNiveauNom, s.nom ?? s.nom_section)
  );

  // Si le niveau change et que la section choisie n'est plus valide -> reset section
  useEffect(() => {
    if (titulaireSectionId == null) return;
    const stillValid = filteredSections.some(
      (s) => (s.id ?? s.id_section) === titulaireSectionId
    );
    if (!stillValid) {
      setTitulaireSectionId(null);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [titulaireNiveauId, sections, niveaux]);
  ...
  const saveProfile = async () => {
    ...
    if (isProf) {
      ...
      if (oneFilled && !bothFilled) {
        return Swal.fire(
          "Champs requis",
          "Sélectionne le niveau ET la section pour la titularisation.",
          "warning"
        );
      }
    }
    ...
    try {
      ...
      if (photoFile) {
        ...
        if (isProf) {
          if (titulaireNiveauId != null) fd.append("titulaireNiveauId", String(titulaireNiveauId));
          if (titulaireSectionId != null) fd.append("titulaireSectionId", String(titulaireSectionId));
        }
        ...
        if (isProf) {
          body.titulaireNiveauId = titulaireNiveauId ?? null;
          body.titulaireSectionId = titulaireSectionId ?? null;
        }
      ...
      // maj des valeurs renvoyées par l’API (ou garde celles du formulaire)
      setTitulaireNiveauId(user?.titulaireNiveauId ?? titulaireNiveauId ?? null);
      setTitulaireSectionId(user?.titulaireSectionId ?? titulaireSectionId ?? null);
      ...
      Swal.fire(
        "Erreur",
        err?.message ||
          "Impossible de mettre à jour le profil. Vérifie qu'aucun autre titulaire n'est déjà défini pour ce niveau + section.",
        "error"
      );
    } finally {
      setSaving(false);
  ...
  return (
    <>
      ...
      <style>{`
        :root{
          --brand:#0b1020; --brand-800:#1e3a8a; --brand-900:#6d28d9;
          --menu-active1:#60a5fa; --menu-active2:#a78bfa;
          --accent:#f37521; --accent-light:#ff8a3d;
          --paper:#ffffff; --paper-2:#fbfdff; --paper-3:#eef2f7; --line:#e5e7eb;
        }
        ...
        .header {
          height: 70px;
          background: linear-gradient(135deg,#0b1020 0%,#1e3a8a 48%,#6d28d9 100%);
          position: relative; display: flex; align-items: center; justify-content: space-between;
          padding: 0 14px; color:#fff; box-shadow:0 10px 24px rgba(2,6,23,.18);
        }
        .header::after{
          content:""; position:absolute; inset:0; pointer-events:none;
          background: radial-gradient(120% 180% at 100% 0%, rgba(96,165,250,.18) 0%, rgba(167,139,250,.16) 35%, rgba(99,102,241,0) 70%);
        }
        .title-box { display:flex; align-items:center; gap:10px; background:#111827; color:#fff; border-radius:12px; padding:8px 10px; }
        .text-left { font-weight:800; letter-spacing:.2px; }
        .box { background: linear-gradient(135deg, var(--accent), var(--accent-light)); color:#111827; font-weight:900; border-radius:10px; padding:6px 12px; box-shadow:0 8px 18px rgba(243,117,33,.35); }
        .burger{ display:none; background:transparent; border:0; width:40px; height:40px; cursor:pointer; }
        .burger-lines, .burger-lines::before, .burger-lines::after{ content:""; display:block; height:2px; background:#fff; border-radius:2px; transition: transform .2s; }
        ...
        .menu{
          ...
          background: linear-gradient(180deg,#0b1020 0%,#0f172a 55%,#111827 100%),
                      radial-gradient(100% 80% at 100% 0%, rgba(96,165,250,.14) 0%, rgba(167,139,250,.12) 40%, rgba(99,102,241,0) 75%);
          background-blend-mode: normal, overlay; box-shadow: 2px 0 18px rgba(2,6,23,.15);
        ...
        .menu-link{ display:block; padding:12px; border-radius:10px; text-decoration:none; color:#fff; font-weight:800; letter-spacing:.2px; transition: transform .06s, box-shadow .18s, background .2s, color .2s; }
        .menu-link:hover{ transform: translateY(-1px); background: rgba(255,255,255,.10); box-shadow:0 10px 20px rgba(96,165,250,.25); }
        .menu-link.active{ color:#0f172a; background: linear-gradient(135deg, var(--menu-active1) 0%, var(--menu-active2) 100%); box-shadow:0 10px 24px rgba(96,165,250,.35); }
        .logout-container{ margin-top:18px; text-align:center; }
        .logout-btn{ width:85%; padding:10px 14px; border:0; border-radius:10px; cursor:pointer; color:#fff; font-weight:900; background: linear-gradient(135deg, #ef4444 0%, #b91c1c 100%); box-shadow:0 10px 22px rgba(239,68,68,.28); transition: transform .06s ease, filter .15s ease; }
        .logout-btn:hover{ transform: translateY(-1px); filter:brightness(1.03); }
        .content{ flex:1; min-width:0; padding:12px; overflow-y:auto;
        ...
        .user-panel{
          ...
          background: linear-gradient(180deg, var(--paper) 0%, var(--paper-2) 60%, var(--paper-3) 100%),
                      radial-gradient(110% 100% at 100% 0%, rgba(243,117,33,.12) 0%, rgba(243,117,33,0) 60%);
          border:1px solid #e5e7eb; border-radius:12px; padding:14px; box-shadow:0 18px 48px rgba(2,6,23,.25);
          transform:translateY(-6px); opacity:0; pointer-events:none; transition: transform .15s, opacity .15s; background-blend-mode: normal, overlay;
        ...
        .user-panel .up-btn{ padding:10px 14px; border:0; border-radius:10px; font-weight:700; cursor:pointer; display:inline-flex; align-items:center; gap:6px; transition: transform .08s, filter .15s, box-shadow .18s; }
        ...
        .up-triple .left{ justify-self:start; } .up-triple .center{ justify-self:center; } .up-triple .right{ justify-self:end; }
        @media (max-width: 1024px){ .menu{ width:230px; min-width:230px; } }
        @media (max-width: 768px){
          .burger{ display:inline-block; }
          .menu{ position:fixed; top:70px; left:-260px; height:calc(100vh - 70px); width:220px; min-width:220px; z-index:20; transition:left .25s ease; }
          .menu.open{ left:0; }
          .content{ width:100%; }
        }
      `}</style>
        ...
        <div className="up-head">
          ...
          {!editEnabled ? (
            ...
            <>
              ...
              {/* === TITULARISATION (après Email) — caché si Proviseur === */}
              {!isProviseur && (
                <div className="fld">
                  <label className="lbl">Titulare de: </label>

                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    <select
                      className="in"
                      value={titulaireNiveauId ?? ""}
                      onChange={(e) => {
                        const val = e.target.value ? Number(e.target.value) : null;
                        setTitulaireNiveauId(val);
                        // la section sera éventuellement reset par l'useEffect si invalide
                      }}
                    >
                      <option value="">— Aucun niveau —</option>
                      {(Array.isArray(niveaux) ? niveaux : []).map((n) => (
                        <option key={n.id} value={n.id}>
                          {n.nom ?? n.nom_niveau ?? `Niveau #${n.id}`}
                        </option>
                      ))}
                    </select>
                    ...
                    <select
                      className="in"
                      value={titulaireSectionId ?? ""}
                      onChange={(e) => {
                        const val = e.target.value ? Number(e.target.value) : null;
                        setTitulaireSectionId(val);
                      }}
                    >
                      <option value="">— Aucune section —</option>
                      {filteredSections.map((s) => {
                        const sid = s.id ?? s.id_section;
                        return (
                          <option key={sid} value={sid}>
                            {s.nom ?? s.nom_section ?? `Section #${sid}`}
                          </option>
                        );
                      })}
                    </select>
